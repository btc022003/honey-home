...
import {
  ...
  delModel,
  modelDetail,
} from '@/services/activity-categories';
...
function ActivityCategories() {
  ...
  const columns: ProColumns<IActivityCategory.ActivityCategory>[] = [
    {
      title: '序号',
      align: 'center',
      hideInSearch: true,
      ...
      render(r, d) {
        return (
          <>
            <Button
              onClick={async () => {
                const detail: IActivityCategory.ActivityCategory = await modelDetail(d.id);
    ...
    <PageContainer>
      ...
      <ModalForm
        ...
        onFinish={async (value: { name: string }) => {
          // console.log(value);
          const saveData: IActivityCategory.ActivityCategory = { ...value };
          if (coverImage) {
