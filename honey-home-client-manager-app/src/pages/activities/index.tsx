...
function Activities() {
  ...
  useEffect(() => {
    allCategories().then((res: any) => {
      ...
      res.data.forEach((item: IActivityCategory.ActivityCategory) => {
        // obj[] = item.
  ...
  const columns: ProColumns<IActivity.Activity>[] = [
    {
      title: '序号',
      align: 'center',
      hideInSearch: true,
    ...
    },
    {
      ...
      dataIndex: 'category',
      render(r, d) {
        return <p>{(d.category as IActivity.Category)?.name}</p>;
      },
      filters: true,
      onFilter: true,
      valueType: 'select',
          ...
          <>
            <Button
              onClick={async () => {
                ...
                form.setFieldsValue({ ...v, category: `${(d.category as IActivity.Activity).id}` });
              }}
              type="primary"
              size="small"
